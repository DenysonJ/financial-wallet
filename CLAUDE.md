...
Financial Microservice, with personal financial organize purposes. Uses Clean Architecture with PostgreSQL, Redis cache, and OpenTelemetry observability.
...
- **Tests**: Unit tests use mocks generated by mockery. E2E tests use TestContainers (Postgres + Redis).
...
- **Stop** — `stop-validate.sh`: build + fmt + vet + tests gate (auto-retry with tiered validation; skipped during active ralph-loop). Swagger and lint are NOT auto-checked — run `/validate` or fix manually.
