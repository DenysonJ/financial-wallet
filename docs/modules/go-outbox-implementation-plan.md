...
que **nao existe hoje no projeto**. Os use cases atuais trabalham com interfaces
simples (Repository) e builder pattern para deps opcionais.

Introduzir UoW so para o outbox criaria inconsistência arquitetural. Se no futuro
o projeto migrar para UoW de forma geral, o outbox module pode adicionar um
...
**Nota**: Isso requer que o Repository exponha um método `CreateWithTx(ctx, tx, entity)`.
O modulo outbox não forca isso — e responsabilidade do serviço consumidor adaptar
...
Múltiplas instancias do relay podem rodar em paralelo graças ao `SKIP LOCKED`:
...
Nao precisa de coordenação, leader election, ou locks distribuídos.
...
O modulo expõe isso via configuração:
...
do módulo, mas a interface `Relay` permite plugar uma implementação CDC.

---

## 13. Cleanup e Retenção
...
### Estrategia Avançada: Table Partitioning
...
## 15. Exemplo de Integração

### 15.1 go.mod do serviço consumidor
