...
if command -v docker &>/dev/null; then
  ...
  if ! echo "$RUNNING" | grep -q 'financial-db'; then
    echo "WARNING: financial-db not running. Run 'make docker-up' in the main project." >&2
  fi
  if ! echo "$RUNNING" | grep -q 'financial-redis'; then
    echo "WARNING: financial-redis not running. Run 'make docker-up' in the main project." >&2
  fi
fi
