...
-- CONCURRENTLY avoids blocking writes on a populated statements table; it also
-- requires NO TRANSACTION at the file level (Goose would otherwise wrap in BEGIN).
CREATE UNIQUE INDEX CONCURRENTLY idx_statements_account_external_id ON statements(account_id, external_id) WHERE external_id IS NOT NULL;
CREATE UNIQUE INDEX CONCURRENTLY idx_statements_unique_reversal ON statements(reference_id) WHERE reference_id IS NOT NULL;
CREATE INDEX CONCURRENTLY idx_statements_account_posted_at ON statements(account_id, posted_at DESC, id DESC);
