...
func fromDomainUser(e *userdomain.User) userDB {
	return userDB{
		...
		PasswordHash: sql.NullString{String: e.PasswordHash, Valid: e.PasswordHash != ""},
		Active:       e.Active,
		CreatedAt:    e.CreatedAt,
		UpdatedAt:    e.UpdatedAt,
	}
...
func (r *UserRepository) UpdatePassword(ctx context.Context, id vo.ID, passwordHash string) error {
	query := `
		UPDATE users SET
			password_hash = $1,
			updated_at = $2
	...
	result, execErr := r.writer.ExecContext(ctx, query, passwordHash, time.Now(), id.String())
	if execErr != nil {
		return execErr
	}
