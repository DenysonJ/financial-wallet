...
func (uc *ImportUseCase) Execute(ctx context.Context, input dto.ImportOFXInput) (*dto.ImportOutput, error) {
	...
	if findErr != nil {
		telemetry.ClassifyError(ctx, span, findErr, "not_found", "statement import failed")
		return nil, findErr
	}
	...
	if ofxErr != nil {
		telemetry.ClassifyError(ctx, span, ofxErr, "invalid_ofx", "statement import failed")
		return nil, ofxErr
	}
	...
	for _, txn := range parseResult.Transactions {
		...
		if mapErr != nil {
			telemetry.ClassifyError(ctx, span, mapErr, "invalid_ofx_transaction", "statement import failed")
			return nil, mapErr
		}
